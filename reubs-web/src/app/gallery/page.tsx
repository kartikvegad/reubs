  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-16">
      ...
      <div className="mt-8 flex flex-wrap gap-2">
        {cats.map((item) => (
          <button
            ...
            type="button"
            onClick={() => setCat(item)}
            className={`rounded-full px-4 py-2 text-sm ${cat === item ? "bg-maroon text-paper" : "border border-gold-soft bg-paper"}`}
          >
            {item}
          </button>
        ))}
      ...
      <div className="mt-10 columns-1 gap-4 sm:columns-2 lg:columns-3">
        {shots.map((shot) => (
          <figure key={`${shot.cat}-${shot.src}`} className="mb-4 break-inside-avoid">
            <Photo src={shot.src} alt={shot.alt} className="aspect-[4/5]" />
            <figcaption className="mt-2 text-xs uppercase tracking-[0.14em] text-muted">{shot.cat}</figcaption>
          </figure>
        ))}
      </div>
    </main>
